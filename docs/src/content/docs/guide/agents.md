...
workmux automatically recognizes built-in agent commands (`claude`, `gemini`, `agy`, `codex`, `opencode`, `kiro-cli`, `vibe`, `pi`, `omp`, `muse`) in pane commands. This means prompt injection works without the `<agent>` placeholder or a matching `agent` config:
...
Each agent receives the prompt using its native format (e.g., Claude uses `--`, Gemini and Antigravity use `-i`, Muse takes it as a positional argument). Auto-detection matches the executable name regardless of flags or path. Just provide a prompt via `-p`, `-P`, or `-e`.
