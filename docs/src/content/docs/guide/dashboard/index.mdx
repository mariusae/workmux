...
import screenshot from "../../../../assets/screenshots/dashboard.webp";
...
<Screenshot src={screenshot} alt="workmux dashboard" framed={false} />
...
- **Worktrees**: Shows all Git or Sapling worktrees with branch/label and agent summary. The preview displays `git log` for Git worktrees and the native Sapling smartlog stack for Sapling worktrees. Press `r` to remove a worktree (kills the agent and removes the worktree; Git branches are also deleted).
