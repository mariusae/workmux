...
To enable tab completions and the shell-level `workmux cd` command, add the following to your shell's configuration file. The generated initialization defines a `workmux` function that delegates normal commands to the binary and uses the shell's `cd` builtin for `workmux cd`.
