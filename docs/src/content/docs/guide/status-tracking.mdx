...
import screenshot from "../../../assets/screenshots/status.webp";
...
<Screenshot
  src={screenshot}
  alt="tmux status showing agent icons"
  framed={false}
/>
...
This detects Claude Code, Antigravity CLI, Copilot CLI, OpenCode, Pi, Oh My Pi, and Muse by checking for their executable or configuration directories, then offers to install the appropriate hooks. For Claude Code, `CLAUDE_CONFIG_DIR` is respected when locating `settings.json`; for Muse, `XDG_CONFIG_HOME` is respected when locating the `muse` config directory. Workmux will also prompt you on first run if it detects an agent without status tracking configured.
...
`workmux setup` installs Antigravity lifecycle hooks in `~/.gemini/config/hooks.json`. `PreInvocation` and `PreToolUse` mark the pane working, and the terminal `Stop` event marks it done.
...
`workmux setup` merges status hooks into your muse `settings.json` (`$XDG_CONFIG_HOME/muse/settings.json`, else `~/.config/muse/settings.json`). `UserPromptSubmit` and `PreToolUse` mark the pane working, and the terminal `Stop` event marks it done. Muse has no `Notification` event, so permission prompts are not tracked as a waiting state.

For manual setup, merge these hook groups into the `hooks` object in your muse `settings.json`:
